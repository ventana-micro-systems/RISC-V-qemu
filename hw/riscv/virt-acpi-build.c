...
struct AcpiExtensionList acpi_extension_list[] = {
    {RISCV_RHCT_ADD_STD_I_TYPE_ZIFENCEIV2, 0},
    {RISCV_RHCT_ADD_STD_I_TYPE_ZIHINTPAUSEV2, 0},
    {RISCV_RHCT_ADD_STD_I_TYPE_ZICSR, 0},
    {RISCV_RHCT_ADD_STD_I_TYPE_ZICMOBASE, 0},
    {RISCV_RHCT_ADD_STD_M_TYPE_ZMMUL, 0},
    {RISCV_RHCT_ADD_STD_A_TYPE_ZAM, 0},
    {RISCV_RHCT_ADD_STD_F_TYPE_ZFINX, 0},
    {RISCV_RHCT_ADD_STD_F_TYPE_ZHINX, 0},
    {RISCV_RHCT_ADD_STD_F_TYPE_ZHINXMIN, 0},
    {RISCV_RHCT_ADD_STD_F_TYPE_ZFH, 0},
    {RISCV_RHCT_ADD_STD_D_TYPE_ZDINX, 0},
    {RISCV_RHCT_ADD_STD_C_TYPE_ZCE, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBA, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBB, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBC, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBE, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBF, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBK, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBP, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBR, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBS, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBKB, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBKC, 0},
    {RISCV_RHCT_ADD_STD_B_TYPE_ZBKX, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKND, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKNE, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKNH, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKSED, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKSH, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKT, 0},
    {RISCV_RHCT_ADD_STD_K_TYPE_ZKR, 0},
    {RISCV_RHCT_ADD_STD_J_TYPE_ZJPM, 0x3FFF},
    {RISCV_RHCT_ADD_STD_T_TYPE_ZTSO, 0},
    {RISCV_RHCT_SUPERVISOR_VM_SVNAPOT, 0},
    {RISCV_RHCT_SUPERVISOR_VM_SVPBMT, 0},
    {RISCV_RHCT_SUPERVISOR_VM_SVINVAL, 0},
    {RISCV_RHCT_SUPERVISOR_TIMER_SSTC, 0},
    {RISCV_RHCT_SUPERVISOR_PMU_SSCOF, 0}
};
                 ...
                 GArray *entry, int base_cpu_id)
{
    ...
    aplic->imsic_info.imsic_size =
        IMSIC_HART_SIZE(0) * vms->soc[socket].num_harts;
    aplic->base_cpu_id = cpu_id;
    aplic->num_harts = vms->soc[socket].num_harts;
    aplic->hart_index = imsic_hart_index.hart_index;
...
build_madt(GArray *table_data, BIOSLinker *linker, RISCVVirtState *vms)
{
    ...
    for (socket = 0; socket < riscv_socket_count(mc); socket++) {
        imsic_s->socket_imsic[socket].imsic_addr =
            vms->memmap[VIRT_IMSIC_S].base +
            socket * ACPI_IMSIC_GROUP_MAX_SIZE;
        imsic_s->socket_imsic[socket].imsic_size =
            IMSIC_HART_SIZE(0) * vms->soc[socket].num_harts;
    }
    imsic_hart_index.lhxw = acpi_num_bits(imsic_max_hart_per_socket);
    imsic_hart_index.hhxw = acpi_num_bits(riscv_socket_count(mc));
    ...
    for (socket = 0; socket < riscv_socket_count(mc); socket++) {
        node_size =
            sizeof(*aplic_s) + (sizeof(uint8_t) * vms->soc[socket].num_harts);
        ...
        aplic_s->imsic_info.imsic_addr = vms->memmap[VIRT_IMSIC_S].base +
            socket * ACPI_IMSIC_GROUP_MAX_SIZE;
        aplic_s->imsic_info.imsic_size =
            IMSIC_HART_SIZE(0) * vms->soc[socket].num_harts;
        aplic_s->base_cpu_id = cpu_id;
        aplic_s->num_harts = vms->soc[socket].num_harts;
        aplic_s->hart_index = imsic_hart_index.hart_index;
    }
...
{
    int rhct_start = table_data->len;
    MachineState *ms = MACHINE(vms);
    int i, socket, acpi_proc_id = 0, node_size;
    /*int j;*/
    RISCVCPU *cpu;
    union AcpiRedtHartHwCap hwcap;
    /*int num_ext = sizeof(acpi_extension_list) / sizeof(struct
     * AcpiExtensionList); */
    AcpiRedtHartCapInfo *rhct_hart_cap;
    ...
    for (socket = 0; socket < riscv_socket_count(ms); socket++) {
        ...
        for (i = 0; i < vms->soc[socket].num_harts; i++) {
            ...
            node_size = sizeof(AcpiRedtHartCapInfo) +
                  sizeof(acpi_extension_list); */
            node_size = sizeof(AcpiRedtHartCapInfo);

            rhct_hart_cap = acpi_data_push(table_data, node_size);
            rhct_hart_cap->length = node_size;
            rhct_hart_cap->version = 1;
            rhct_hart_cap->acpi_proc_id = acpi_proc_id++;
            rhct_hart_cap->isa = cpu->env.misa;
            hwcap.mmu_type = RISCV_HART_CAP_MMU_TYPE_48;
            hwcap.aia_enabled = 1;
            rhct_hart_cap->hart_hwcap = hwcap.hw_cap;
            /* Enable for v2 version
               for (j = 0; j < num_ext; j++) {
               rhct_hart_cap->extensions[j].extension_type =
               acpi_extension_list[j].extension;
               rhct_hart_cap->extensions[j].attr =
               acpi_extension_list[j].attributes;
               }
             */
        }
    }

    build_header(linker, table_data, (void *) (table_data->data + rhct_start),
                  "RHCT", table_data->len - rhct_start, 2, vms->oem_id,
                  vms->oem_table_id);
}
...
build_dsdt(GArray *table_data, BIOSLinker *linker, RISCVVirtState *vms)
{
    ...
    build_header(linker, table_data,
                  (void *) (table_data->data + table_data->len -
                            dsdt->buf->len), "DSDT", dsdt->buf->len, 2,
